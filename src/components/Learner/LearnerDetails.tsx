    ...
    <div className="border border-gray-300 rounded-lg p-4 mb-3">
      ...
      {expanded && (
        <div className="mt-3 space-y-2 text-sm">
          ...
          <p><span className="font-medium">Grade:</span> {topic.grade}%</p>
          <p><span className="font-medium">Tutor ID:</span> {topic.tutorId}</p>
        </div>
      )}
    </div>
  );
      ...
      <div>
        ...
        {student.awards.length > 0 ? (
          student.awards.map((award, index) => (
            <div key={index} className="border border-gray-300 rounded p-4 mb-3 bg-yellow-50">
              <p><span className="font-medium">Award:</span> {award.type}</p>
              <p><span className="font-medium">Topic:</span> {award.topic}</p>
            </div>
          ))
        ) : (
